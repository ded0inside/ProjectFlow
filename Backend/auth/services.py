...
async def get_token(data, db):
    user = db.query(UserModel).filter((UserModel.email == data.username) or (UserModel.username == data.username)).first()
    
    if not user:
        raise HTTPException(
            status_code=400,
            detail="Email or username is not registered with us.",
